" The Pyte theme, enhanced to make comments more prominent.
...
let colors_name = "pyte-telotortium"
...
hi Comment gui=italic guifg=#5342B3
" Use italics for comments in terminal if possible (checked by testing for the
" italic terminal option).
let s:hookvar = 'g:CSApprox_hook_pytetelotortium_post'
if !exists(s:hookvar)
    let {s:hookvar} = []
elseif type({s:hookvar}) == type("")
    let {s:hookvar} = [{s:hookvar}]
else
    " Hook is already assigned a list.
endif
let s:hook = 'if &t_ZH == "[3m" | hi Comment cterm=italic | endif'
if index({s:hookvar}, s:hook) == -1
    call add({s:hookvar}, s:hook)
endif
